...
export async function run(root: HTMLElement): Promise<void> {
  ...
  await mountTaskApp({
    root,
    task_id: "H000018-oddball-mmn",
    task_name: "Oddball Task (MMN/P3)",
    task_description: "HTML preview aligned to local psyflow Oddball MMN/P3 procedure and parameters.",
    settings,
    subInfo,
    stimBank,
    buildTrials: (): CompiledTrial[] => {
      ...
      const generationSettings = resolve_generation_settings(conditionLabels, conditionGenerationConfig);
      ...
      for (let blockIndex = 0; blockIndex < totalBlocks; blockIndex += 1) {
        ...
        const block = new BlockUnit({
          ...
          func: generate_oddball_conditions,
          ...
          buildWaitTrial(
            ...
            [
              (_snapshot: TrialSnapshot, runtime: RuntimeView) => {
                const metrics = summarizeBlock(runtime.getReducedRows(), block.block_id);
      ...
      compiledTrials.push(
        buildWaitTrial(
          ...
          [
            (_snapshot: TrialSnapshot, runtime: RuntimeView) => {
              const metrics = summarizeOverall(runtime.getReducedRows());
              return stimBank.get_and_format("good_bye", metrics);
            }
          ]
        )
      );
